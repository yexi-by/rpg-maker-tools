...
def test_att_mz_skill_requires_parallel_subagents_when_available() -> None:
    ...
    required_phrases = [
        ...
        "`quality-fix-template.json`：由 `export-quality-fix-template` 生成",
        "`reset-translations.json`：顶层必须是 `{\"location_paths\": [\"<定位路径>\"]}`",
        "禁止用空 `translation_lines` 当重置信号",
        "`japanese-residual-rules.json`：顶层是 `{location_path: {allowed_terms, reason}}`",
        ...
        "`<拡張説明:...>`、`<ExtendDesc:...>`",
        "`<upgrade:...>`、`<ChainSkill:...>`、`<EquipState:...>`",
        "`quality-report --json` 返回的 `quality_error_items`",
        ...
        "推荐先用 `export-quality-fix-template --json` 生成骨架",
