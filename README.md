...
# 编辑 setting.toml，填入模型服务地址和 API Key
# 然后注册你的游戏
...
完整从零到翻译注入的流程指南见 `docs/cli-project-usage.md`。
...
每个配置项都可通过 CLI 参数覆盖，详见 `docs/cli-project-usage.md` 的配置体系章节。
...
也可以在翻译时通过 CLI 直接传入规则字符串。详见 `docs/custom-placeholder-rules.md`。
...
每个外部 Agent 任务的参考提示词见 `docs/` 目录下的对应文档。
