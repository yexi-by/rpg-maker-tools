...
- 术语表、插件规则和事件指令规则必须由当前 Agent 自己分析导出文件；有内容就生成导入 JSON，没有内容就记录确认结论，不转交给另一个 Agent，除非用户明确要求。
...
症状：当前游戏数据库中术语表、插件规则或事件指令规则为空，且尚未确认游戏本身是否存在对应内容。

门禁：

- 三类数据都是翻译前强制检查项。
- 强制的是导出、分析、确认和验收，不是凭空产出非空规则。
- 只要数据库里任一类为空，必须导出对应文件，由当前 Agent 自己分析。
- 分析后存在对应内容：生成导入文件，执行 `import-*`，再用 `doctor --game "<游戏标题>" --json` 或 `quality-report --game "<游戏标题>" --json` 验收计数。
- 分析后确认游戏本身没有对应内容：不要编造规则或术语；在交付报告中记录“已检查，确认无对应内容”，然后允许继续，即使数据库计数仍为 0。
- 如果不确定是真的没有，还是 Agent 没看懂结构，必须暂停向用户反馈，不能把“不确定”当成“无对应内容”。
- 验收或确认完成后删除本轮导出的临时分析文件和规则产物。
- 主流程只能读取数据库，不读取临时 JSON。

合法空结果：

- 术语表：导出的 `name_registry.json` 里 `speaker_names` 和 `map_display_names` 都为空，表示当前游戏没有可登记的名字框和地图显示名。
- 插件规则：导出的 `plugins.json` 是空数组，或所有插件参数都没有玩家可见文本，只包含资源路径、开关、数字、脚本、枚举、颜色、坐标、布局和内部标识。
- 事件指令规则：导出的 `event-commands.json` 中所有编码数组都为空，或所有导出的参数结构都没有玩家可见文本叶子。

合法空结果的处理方式：

1. 不生成为了通过计数而存在的空壳规则。
2. 不执行没有意义的 `import-*`。
3. 在交付报告中分别写清楚哪一类已检查、为什么确认没有对应内容。
4. 继续执行后续翻译流程。

先检查数据库状态：

```bash
uv run python main.py doctor --game "<游戏标题>" --json --no-check-llm
uv run python main.py quality-report --game "<游戏标题>" --json
```

需要关注：

- `name_context_imported` 为 `false`，或 `name_context_total_count` 为 `0`：执行术语表检查流程。
- `plugin_rule_count` 为 `0`：执行插件规则检查流程。
- `event_command_rule_count` 为 `0`：执行事件指令规则检查流程。
...
1. 阅读 `name_registry.json`，确认顶层只有 `speaker_names` 和 `map_display_names`。
2. 分批阅读 `speaker_contexts/*.json`，根据对白上下文判断名字、称呼、身份、声音或地图名。
3. 只填写 `name_registry.json` 的 value；保持所有 key 不变。
4. 同一角色、同一势力、同一称呼体系必须统一译名。
5. 如果是普通身份称呼，正常翻译；如果是角色名，结合上下文给稳定中文名。
6. 不新增字段，不写 note，不把路径、上下文文件名或来源说明写进 JSON。

验收：

- `name_registry.json` 能被 JSON 解析。
- 顶层只有 `speaker_names` 和 `map_display_names`。
- 所有原文 key 完全保留。
- 不存在空译名，除非原文本身不应翻译。
- 导入后 `doctor` 中 `name_context_imported` 为 `true`，`quality-report` 中 `name_context_total_count` 大于 `0` 且 `name_context_filled_count` 等于可填写条目数。
- 如果导出的 `name_registry.json` 中 `speaker_names` 和 `map_display_names` 都为空，确认当前游戏没有名字框和地图显示名术语；不导入空数据，在交付报告中记录该结论。
...
1. 阅读 `plugins.json` 的 `$plugins` 数组结构。
2. 分插件分析 `parameters` 中的字符串叶子。
3. 只选择玩家可见文本字段。
4. 排除路径、文件名、脚本、枚举、布尔值、数字、颜色、坐标、布局参数、资源名和内部标识。
5. 生成 `plugin-rules.json`，格式为插件名到 JSONPath 数组的对象。
...
- `plugin-rules.json` 能被 JSON 解析。
- 顶层是对象，key 是插件名，value 是 JSONPath 字符串数组。
- JSONPath 必须指向字符串叶子。
- 规则不包含明显资源路径、脚本、布尔值、数字和布局字段。
- 导入后 `doctor` 或 `quality-report` 中 `plugin_rule_count` 大于 `0`。
- 如果 `plugins.json` 中没有插件，或当前 Agent 确认所有插件参数都没有玩家可见文本，不生成无意义规则；在交付报告中记录“插件规则已检查，确认无可翻译插件文本”。
...
1. 阅读 `event-commands.json`，按事件指令编码分组分析 `parameters` 结构。
2. 对同一编码下不同结构，用 `match` 区分固定参数值。
3. 只选择玩家可见文本叶子。
4. 排除脚本、资源路径、枚举、布尔值、数字、颜色、坐标和布局参数。
5. 生成 `event-command-rules.json`，顶层 key 是事件指令编码字符串，value 是规则对象数组。
...
- `event-command-rules.json` 能被 JSON 解析。
- 顶层是对象，编码 key 是字符串。
- 每个规则对象只有必要的 `match` 和 `paths`。
- `paths` 指向玩家可见文本字符串叶子。
- 导入后 `doctor` 或 `quality-report` 中 `event_command_rule_count` 大于 `0`。
- 如果 `event-commands.json` 为空，或当前 Agent 确认事件指令参数中没有玩家可见文本，不生成无意义规则；在交付报告中记录“事件指令规则已检查，确认无可翻译事件指令参数文本”。

统一收尾：

- 导入失败时先修正规则文件，不绕过校验。
- 需要机器可读诊断时，优先使用 `--json` 或 `--output`，不要从终端彩色日志里猜状态。
- 三类数据全部导入并验收通过，或确认游戏本身没有对应内容后，删除 `<外部临时目录>` 下的 `name-context`、`plugins.json`、`plugin-rules.json`、`event-commands.json`、`event-command-rules.json` 等临时分析产物。
- 只保留用户明确要求保留的质量报告或交付摘要。
...
- 自定义控制符已覆盖。
- 术语表、插件规则和事件指令规则已导入；如果游戏本身没有对应内容，已完成检查并在交付报告中记录确认结论。
- 目标游戏目录可写。
...
- 看到报错就无限重跑。
- 没有扫描自定义控制符就开始翻译。
- 数据库中术语表、插件规则或事件指令规则为空，且没有完成对应内容检查时开始翻译。
- 为了让计数非空而编造不存在的术语、插件规则或事件指令规则。
- 把没有看懂结构的情况当成“游戏没有对应内容”。
- 把导出的 name-context、plugins 或 event-commands 交给另一个 Agent 分析，而不是当前 Agent 自己处理。
- 把 API Key 写进 CLI 参数。
- 直接读取外部临时 JSON 参与主流程。
