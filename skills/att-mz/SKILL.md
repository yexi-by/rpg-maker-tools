...
| `import-name-context --game <游戏标题> --input <术语表> --json` | 填好的 `name_registry.json` | key 未改、只填 value | 导入名字框和地图名术语 | `status` 为 `ok` | 修术语表结构或空值策略后重跑 |
...
- 导入：`import-name-context --game <游戏标题> --input <工作区>/name-context/name_registry.json --json`。
...
- `<项目目录>`、`<工作区>`、`<游戏标题>`。
- 子代理角色名，例如 `name-context`、`plugin-rules`、`event-command-rules`、`note-tag-rules`。
- 输入文件清单和输出文件路径。
