...
- `--json` 命令的 stdout 只读取最终 JSON；`translate`、`write-back`、`write-terminology` 等长任务会在 stderr 持续输出无 ANSI 文本进度条，显示已完成数量、百分比、已用时间、预计剩余时间和当前状态。
...
5. 第一轮子代理只处理术语候选：主代理按术语字段拆分 `terms.json`，把独立候选文件交给多个子代理；子代理只能写各自候选文件，不能直接写 `terminology/terms.json`。主代理必须等待全部结果、逐项审查信达雅和译名统一、亲自修改并合并后，才执行 `import-terminology --json`。
...
15. 清理 `<工作区>`；如果工作区由 `prepare-agent-workspace` 生成，优先用 `cleanup-agent-workspace --workspace <工作区> --json`。
...
- `terminology/terms.json`：术语表，只填写 value。
...
`prepare-agent-workspace` 会优先把 CLI 已保存到当前游戏状态里的术语表、插件规则、事件指令规则、Note 标签规则和占位符规则回填到工作区。新游戏或未保存规则时，工作区仍会给出空对象或候选草稿，供主代理分析填写。占位符候选和草稿只是前置参考，最终规则必须在术语表和三类外部规则保存后重新生成或逐条确认。
...
| 术语表工程 | `terminology/terms.json`、术语上下文、术语拆分文件 | 主代理拆分术语字段并发起第一轮候选子代理任务；主代理严审信达雅、统一译名、亲自修改并合并 | 已通过 CLI 保存的术语表 |
...
| 验收导入 | 术语表、三类外部规则文件和占位符规则文件 | 逐个运行 `validate-* --json` 或导入命令，通过后再保存到项目数据库 | 当前游戏有效规则 |
...
| `import-terminology --game <游戏标题> --input <术语表> --json` | 填好的 `terms.json` | key 未改、只填 value | 导入名字框、地图名、数据库名称和系统类型术语 | `status` 为 `ok` | 修术语表结构或空值策略后重跑 |
...
| `reset-translations --game <游戏标题> --all --json` | 当前游戏状态和当前提取范围 | 用户明确要求完整重译已完成游戏 | 删除当前提取范围内全部已保存译文记录，让 `translate` 重新处理 | `status` 不是 `error`，`summary.mode=all`，`summary.requested_count` 与当前提取量可解释 | 如果 `reset_count=0`，先确认是否本来没有已保存译文，不要绕过 CLI |
...
- `terminology/terms.json`：顶层固定为术语类别对象，包括 `speaker_names`、`map_display_names`、角色、职业、技能、物品、装备、敌人、状态和系统类型术语。只填写已有 key 对应的 value；不改 key，不新增字段，不写 note，不把样本文件路径写入 value。不能确定译名时保留空字符串并在最终报告说明。
...
- `quality-fix-template.json`：这是“检查没通过译文的修复表”，由 `export-quality-fix-template` 生成，顶层同样是 `{location_path: 条目对象}`，定位键仍然只需原样保留。只改 `translation_lines`，也就是中文译文行；已保存但仍有问题的文本会预填当前译文，模型翻过但检查没通过的文本会优先预填模型临时译文，没有临时译文时为空数组。
...
术语表和三类外部规则在翻译前都必须导出、分析、确认、验收。强制的是“确认”，不是强制产出非空规则。

### 术语表

- 输入：`terminology/terms.json`、`terminology/contexts/speakers/*.json` 和 `terminology/contexts/database_terms.json`。
- 术语表翻译必须由主代理亲自把关；子代理只能提供分字段候选译名，不能直接把结果写入最终 `terminology/terms.json`。
...
- 主代理必须亲自修改候选结果，合并到 `terminology/terms.json`，再做全量检查：空译名、假名残留、机械音译残渣、同一原文跨类别冲突、关键术语口径。
...
- 导入：`import-terminology --game <游戏标题> --input <工作区>/terminology/terms.json --json`。
...
术语表翻译必须由主代理亲自把关。第一轮子代理只产出候选译名，不得直接写最终 `terminology/terms.json`，不得保存为当前游戏术语表。
...
- 只有主代理审查通过后，才能运行 `import-terminology --game <游戏标题> --input <工作区>/terminology/terms.json --json`。
...
主代理合并术语表时必须保留 `terminology/terms.json` 的完整顶层类别和 key 集合；候选文件只能作为输入，不能代替最终导入文件。
