...
5. 并行分析术语表、插件规则、事件指令规则和 Note 标签规则，主 Agent 二次审查后逐项校验并导入。
6. 主 Agent 基于当前会进入正文翻译的完整文本集合，亲自生成、审查、覆盖扫描、校验并导入占位符规则。
...
13. `quality-report --json` 没有错误，并且用户允许写回后，执行 `write-back --game <游戏标题> --json`。普通写回不会覆盖字体；只有用户另外明确允许覆盖字体时，才执行 `write-back --game <游戏标题> --confirm-font-overwrite --json`。
...
- `manifest.json`：清理清单。
- `placeholder-candidates.json`：初始候选控制符报告，只供主 Agent 参考；最终占位符规则必须在四类外部规则导入后重新确认。
- `placeholder-rules.json`：初始占位符规则草稿；不能跳过主 Agent 最终审查直接导入。
...
`prepare-agent-workspace` 会优先把当前游戏数据库中已经通过 CLI 导入的术语表、插件规则、事件指令规则、Note 标签规则和占位符规则回填到工作区。新游戏或未导入规则时，工作区仍会给出空对象或候选草稿，供 Agent 分析填写。占位符候选和草稿只是前置参考，最终规则必须由主 Agent 在四类外部规则导入后重新生成或逐条确认。
...
| 外部分析 | 工作区候选文件、规则草稿 | 由主 Agent 派发子代理筛选可翻译内容，主 Agent 二次审查 | 术语表、插件规则、事件指令规则、Note 标签规则 |
| 占位符收束 | 当前已导入规则、当前会进入正文翻译的完整文本集合 | 主 Agent 统一扫描、生成、审查、校验占位符规则 | 当前游戏数据库内的有效占位符规则 |
| 验收导入 | 四类外部分析文件和占位符规则文件 | 逐个运行 `validate-* --json`，通过后再 `import-*` | 当前游戏数据库内的有效规则 |
...
| `import-placeholder-rules --game <游戏标题> --input <规则文件> --json` | 已校验且覆盖扫描通过的规则文件 | validate 与 scan 都已通过 | 导入当前游戏数据库 | `status` 为 `ok` 或只有可接受空结果 warning | 回到 validate/scan 修规则，不直接改库 |
...
| `validate-agent-workspace --game <游戏标题> --workspace <工作区> --json` | 完整工作区 | 四类子代理文件和主 Agent 占位符规则已复核 | 总体验收工作区可导入性，并阻断未覆盖当前正文控制符的占位符规则 | 无 `errors` | 逐项修工作区 JSON 后重跑 |
...
- `terminology/terms.json`：顶层固定为术语类别对象，包括 `speaker_names`、`map_display_names`、角色、职业、技能、物品、装备、敌人、状态和系统类型术语。Agent 只填写已有 key 对应的 value；不改 key，不新增字段，不写 note，不把样本文件路径写入 value。不能确定译名时保留空字符串并在最终报告说明。
...
- `note-tag-rules.json`：顶层必须是对象，格式为 `{data文件名或文件模式: [note标签名, ...]}`。合法示例：`{"<data文件名>.json": ["<玩家可见说明标签>"], "<地图文件模式>": ["<玩家可见名牌标签>"]}`。只写候选里真实存在的精确标签名，不支持标签正则；文件名可用 `<地图文件模式>` 这类模式覆盖同类地图文件；空结果使用 `{}`，但必须说明已检查候选。禁止选择脚本、公式、资源名、ID、升级材料、布尔/枚举、装备状态编号、连锁技能编号和纯系统标签。
...
- 占位符规则由主 Agent 亲自处理，不派发给子代理。
- 必须先完成术语表、插件规则、事件指令规则和 Note 标签规则的二次审查、校验与导入，再收束占位符规则。
...
- 原文确实没有某类术语时，对应类别保持空对象；存在术语但无法确定译名时保留空字符串并在交付报告说明。
...
- 排除机器协议：脚本、公式、资源名、ID、升级材料、布尔/枚举、装备状态编号、连锁技能编号、开关名、坐标、文件路径等标签。只要样例值主要是数字、公式、路径、枚举或内部标识，就不要选。
...
- `terminology` 子代理：读取 `terminology/terms.json`、`terminology/contexts/speakers/*.json` 和 `terminology/contexts/database_terms.json`，只写 `terminology/terms.json`。
...
| 子代理 | 输入 | 逻辑 | 输出 |
| --- | --- | --- | --- |
| `terminology` | `terminology/terms.json`、`terminology/contexts/speakers/*.json`、`terminology/contexts/database_terms.json` | 统一名字框、地图名、数据库名称和系统类型术语；只填写 value；不新增字段和说明 | 只写 `terminology/terms.json`，key 保持不变 |
...
- 主 Agent 必须运行插件规则、事件指令规则和 Note 标签规则对应的 `validate-*` 命令；术语表由导入命令和 `validate-agent-workspace` 检查。
- 主 Agent 必须在校验通过后执行对应 `import-* --json` 命令，并读取 `status` 和 `summary`。
- 四类子代理全部导入后，主 Agent 必须重新运行 `build-placeholder-rules`，亲自审查、校验、覆盖扫描并导入占位符规则。
...
- `<项目目录>`、`<工作区>`、`<游戏标题>`。
- 子代理角色名，例如 `terminology`、`plugin-rules`、`event-command-rules`、`note-tag-rules`。
- 输入文件清单和输出文件路径。
...
- `terminology`：说明只能填写 `terms.json` 的 value，不能改 key、不能新增字段、不能写 note；必要时读取 `terminology/contexts/speakers/*.json` 和 `terminology/contexts/database_terms.json` 判断术语一致性。
...
- `note-tag-rules`：说明 `note-tag-rules.json` 必须是 `{data文件名或文件模式: [note标签名, ...]}`；只选玩家可见 Note 标签值，排除脚本、公式、资源名、ID、枚举、装备状态编号、连锁技能编号等机器协议。
...
```text
输入：读取 <工作区>/terminology/terms.json、<工作区>/terminology/contexts/speakers/*.json 和 <工作区>/terminology/contexts/database_terms.json。
逻辑：根据对白样本和术语语义上下文统一名字框、地图名、数据库名称和系统类型术语；只填写已有 key 的 value。
输出：只写 <工作区>/terminology/terms.json，不改 key，不新增字段。
空结果：某类术语为空时保持空对象；存在术语但无法确定译名时保留空 value 并说明。
完成报告：说明填写数量、保留空值原因、术语一致性风险、建议运行 validate-agent-workspace --json。
```
...
```text
输入片段 terms: {"speaker_names": {"<角色原名>": "", "<声音称呼原文>": ""}, "map_display_names": {"<地图原名>": ""}, "skill_names": {"<技能原名>": ""}}
参考 speakers: {"name": "<角色原名>", "dialogue_lines": ["<对白样本>"]}
参考 database_terms: {"category": "skill_names", "source_text": "<技能原名>", "context_lines": ["<技能说明样本>"]}
正确输出: {"speaker_names": {"<角色原名>": "<角色译名>", "<声音称呼原文>": "<声音称呼译名>"}, "map_display_names": {"<地图原名>": "<地图译名>"}, "skill_names": {"<技能原名>": "<技能译名>"}}
错误输出: {"speaker_names": {"引路人": "案内人"}, "note": "看起来像 NPC"}
```

判断逻辑：必须读取 `terminology/contexts/speakers/*.json` 和 `terminology/contexts/database_terms.json`，根据上下文统一称号、声音变体、地图名和数据库术语；只填已有 value，不改 key，不加 note。完成报告必须写明读取了多少个 `terminology/contexts/speakers/*.json`、哪些译名由上下文决定、哪些 value 保留空以及原因。校验命令：`validate-agent-workspace --game <游戏标题> --workspace <工作区> --json`。
