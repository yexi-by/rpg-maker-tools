---
name: att-mz
description: Use this skill when operating A.T.T MZ, the RPG Maker MZ autonomous translation toolkit, for project discovery, game registration, Agent workspace analysis, placeholder rules, terminology, plugin rules, event command rules, note tag rules, translation loops, quality checks, manual completion, and write-back gating.
---
...
13. `quality-report --json` 无阻断问题，并且用户允许写回后，执行 `write-back --game <游戏标题> --json`。
...
- `note-tag-rules.json`：Note 标签规则草稿，默认 `{}`。
...
| 验收导入 | 五类外部分析产物 | 逐个运行 `validate-* --json`，通过后再 `import-*` | 当前游戏数据库内的有效规则 |
...
| `prepare-agent-workspace --game <游戏标题> --output-dir <工作区> --json` | 游戏标题、工作区目录 | 游戏已注册 | 导出五类外部分析输入和 `manifest.json` | 工作区文件存在，`summary.workspace` 指向目标目录 | 删除不完整工作区后重跑 |
...
| `validate-agent-workspace --game <游戏标题> --workspace <工作区> --json` | 完整工作区 | 五类产物已由主 Agent 复核 | 总体验收工作区可导入性；缺 `note-tag-rules.json` 是阻断错误 | 无 `errors` | 逐项修工作区 JSON 后重跑 |
...
| `export-quality-fix-template --game <游戏标题> --output <文件> --json` | 游戏标题、输出文件 | `quality-report` 有质量问题明细 | 从质量报告生成可导入的修复 JSON 骨架，预填当前译文或候选译文 | 输出文件存在，`summary.exported_count` 可解释 | 修 `translation_lines` 后走 `import-manual-translations`；禁止手拼数据库 UPDATE |
...
| `reset-translations --game <游戏标题> --input <文件> --json` | `{"location_paths": [...]}` 文件 | 明确需要让坏译文回到 pending | 精确删除这些路径的已入库译文缓存 | `status` 为 `ok`，`summary.reset_count` 可解释 | 非法路径会整体阻断；修输入文件，不用空译文伪造重置 |
...
- `quality-fix-template.json`：由 `export-quality-fix-template` 生成，顶层同样是 `{location_path: 条目对象}`。Agent 只改 `translation_lines`；已入库问题会预填当前译文，质量错误会优先预填候选译文，没有候选时为空数组。
- `reset-translations.json`：顶层必须是 `{"location_paths": ["<定位路径>"]}`。只用于显式重置坏译文；数组不能为空，路径必须来自当前提取范围，禁止用空 `translation_lines` 当重置信号。
- `japanese-residual-rules.json`：顶层是 `{location_path: {allowed_terms, reason}}`。`allowed_terms` 是允许原样保留的日文片段字符串数组；`reason` 必须说明原因，例如 `credits`、`staff_name`、`proper_noun`、`brand_name`。只在模型或人工 Agent 判断该片段确实不应翻译时使用；禁止用它掩盖整句漏翻，禁止在 `pending-translations.json` 内新增例外字段。
...
- 如果 CLI 报 `疑似控制符不一致`、`placeholder_risk` 或 `CUSTOM_UNEXPECTED`，先逐条比较 `original_lines` 与 `translation_lines` 的控制片段，修补译 JSON 后重跑导入或质量报告；禁止直接改数据库，禁止把失败条目清空。
...
- `quality-report --json` 返回的 `quality_error_items` 是最新翻译运行的失败明细；其中 `error_type=日文残留` 时，先判断译文是漏翻还是确需保留原文。
...
- 质量报告出现 `quality_error_items`、`placeholder_risk_items`、`overwide_line_items` 或 `japanese_residual_items` 时，优先运行 `export-quality-fix-template --game <游戏标题> --output <工作区>/quality-fix-template.json --json`，只修 `translation_lines` 后用 `import-manual-translations` 导入，减少手工拼 JSON。
- `overwide_line` 是写回阻断问题，必须修复到 `quality-report --json` 无错误后才能写回。
- `japanese_residual` 表示译文仍有疑似日文残留。优先判断是不是漏翻；如果是漏翻，修 `translation_lines` 后导入。只有致谢名单、Staff 名、作品名、品牌名、游戏内专有名词等确实无需翻译的片段，才写入 `japanese-residual-rules.json` 并走 validate/import 例外流程。
- 如果模型明确认为某个日文片段保留原文比硬翻更准确，Agent 可以通过日文残留例外规则放行；必须限制到具体 `location_path` 和具体 `allowed_terms`，并填写 `reason`。
...
- 只有确认为坏译文需要重新交给 `translate` 时，才使用 `reset-translations --game <游戏标题> --input <工作区>/reset-translations.json --json`。该命令只接受 `{"location_paths": [...]}`，非法路径会整体阻断，不能用空 `translation_lines` 伪造重置。
...
- `japanese_residual_rules_invalid`：优先检查顶层 key 是否是当前 pending 或已入库条目的 `location_path`，`allowed_terms` 是否为非空字符串数组且片段出现在当前条目原文或译文中，`reason` 是否非空。
...
- `quality-report --json` 返回可修复明细时，推荐先用 `export-quality-fix-template --json` 生成骨架；只有该命令输出为空或 CLI 行为异常时，才手工整理同格式 JSON。
- 需要让坏译文回到 pending 时，必须用 `reset-translations` 的 `location_paths` 显式文件；禁止把 `translation_lines` 写成空数组来绕过导入校验。
...
- 直接改游戏 `data/*.json` 的 `note` 字段绕过 `note-tag-rules`、翻译缓存和写回门禁。
