...
| 组件 | 用途 |
|------|------|
| A.T.T MZ CLI | 当前系统对应的 `dist/` 可执行文件 |
| [Rust](https://rustup.rs/) stable | 仅开发或重新构建时需要 |
| Zig 官方工具 | `xtask dist` 会按需下载，用于 Linux/macOS 交叉编译 |
| OpenAI 兼容模型服务 | 翻译后端 |
...
`<att-mz>` 表示当前系统对应的可执行文件路径。开发或重新打包时执行：
...
`xtask dist` 会把 Windows、Linux、macOS ARM64 三端可执行文件输出到 `dist/`。
...
```powershell
<att-mz> --help
<att-mz> doctor --no-check-llm --json
<att-mz> add-game --path <游戏目录> --json
<att-mz> list --json
```
...
<att-mz> doctor --no-check-llm --json
<att-mz> add-game --path <游戏目录> --json
<att-mz> doctor --game <游戏标题> --no-check-llm --json
<att-mz> prepare-agent-workspace --game <游戏标题> --output-dir <工作区> --json
```
...
```powershell
<att-mz> import-terminology --game <游戏标题> --input <工作区>/terminology/field-terms.json --glossary-input <工作区>/terminology/glossary.json --json
```
...
```powershell
# 插件规则
<att-mz> validate-plugin-rules --game <游戏标题> --input <工作区>/plugin-rules.json --json
<att-mz> import-plugin-rules --game <游戏标题> --input <工作区>/plugin-rules.json --json

# 事件指令规则
<att-mz> validate-event-command-rules --game <游戏标题> --input <工作区>/event-command-rules.json --json
<att-mz> import-event-command-rules --game <游戏标题> --input <工作区>/event-command-rules.json --json

# Note 标签规则
<att-mz> validate-note-tag-rules --game <游戏标题> --input <工作区>/note-tag-rules.json --json
<att-mz> import-note-tag-rules --game <游戏标题> --input <工作区>/note-tag-rules.json --json
```
...
```powershell
<att-mz> build-placeholder-rules --game <游戏标题> --output <工作区>/placeholder-rules.json --json
<att-mz> validate-placeholder-rules --game <游戏标题> --input <工作区>/placeholder-rules.json --json
<att-mz> scan-placeholder-candidates --game <游戏标题> --input <工作区>/placeholder-rules.json --json
<att-mz> import-placeholder-rules --game <游戏标题> --input <工作区>/placeholder-rules.json --json
```
...
```powershell
<att-mz> validate-agent-workspace --game <游戏标题> --workspace <工作区> --json
```
...
```powershell
# 小批量试跑
<att-mz> translate --game <游戏标题> --max-batches 1 --json

# 查看状态
<att-mz> translation-status --game <游戏标题> --json
<att-mz> quality-report --game <游戏标题> --json

# 全量翻译
<att-mz> translate --game <游戏标题> --json

# 写回
<att-mz> write-back --game <游戏标题> --json
```
...
```powershell
<att-mz> export-untranslated-translations --game <游戏标题> --output <工作区>/pending-translations.json --json
<att-mz> import-manual-translations --game <游戏标题> --input <工作区>/pending-translations.json --json
```

分批或抽样填写：

```powershell
<att-mz> export-pending-translations --game <游戏标题> --limit N --output <文件> --json
```
...
```powershell
<att-mz> export-quality-fix-template --game <游戏标题> --output <工作区>/quality-fix-template.json --json
<att-mz> import-manual-translations --game <游戏标题> --input <工作区>/quality-fix-template.json --json
```
...
```powershell
<att-mz> reset-translations --game <游戏标题> --input <工作区>/reset-translations.json --json
```
...
```powershell
<att-mz> reset-translations --game <游戏标题> --all --json
```
...
```powershell
<att-mz> validate-japanese-residual-rules --game <游戏标题> --input <工作区>/japanese-residual-rules.json --json
<att-mz> import-japanese-residual-rules --game <游戏标题> --input <工作区>/japanese-residual-rules.json --json
```
...
```powershell
<att-mz> restore-font --game <游戏标题> --json
```
