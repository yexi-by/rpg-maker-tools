...
- `command_code`：事件指令编码
- `parameter_filters`：可选，用于缩小匹配范围的参数过滤条件（按 `index` 和 `value` 过滤）
- `path_templates`：该指令中可翻译文本的 JSONPath 模板数组
...
RPG Maker MZ 中，事件指令 101（显示文字）的第四个参数指定说话人名字框，MapXXX 的 `displayName` 指定地图显示名。这些名字通常是日文，需要在翻译正文前确定标准译名，以保证全游戏名字一致。
...
long_text_line_width_limit = 47           # 长文本单行宽度上限
line_width_count_pattern = "[\\u3400-\\u4DBF\\u4E00-\\u9FFF\\uF900-\\uFAFF]"  # 宽度计数字符正则
