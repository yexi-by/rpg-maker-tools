...
from app.models.game_data import CommonEvent, EventCommand, MapData, Troop
...
def _check_command_list(commands: list[EventCommand], location_info: str) -> None:
    ...
    for command_index, command in enumerate(commands):
        if command.code == Code.TEXT and previous_code not in (Code.NAME, Code.TEXT):
            raise ValueError(
                "对话探针检查失败: "
                f"{location_info} 中发现孤立的 Code 401, "
                f"当前指令索引 {command_index}, 前置指令 Code 为 {previous_code}"
            )
