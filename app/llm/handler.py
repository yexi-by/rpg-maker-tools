...
class LLMHandler:
    ...
    def register_service(self, *, llm_setting: LLMSettings) -> None:
        ...
        每个被注册的服务都会被赋予一个唯一的逻辑名称（如 "text" 或 "plugin_text"），
        ...
        if llm_setting.provider_type == "openai":
            openai_client = AsyncOpenAI(
                api_key=llm_setting.api_key,
                base_url=llm_setting.base_url,
                timeout=llm_setting.timeout,
            )
            self.services[llm_setting.name] = OpenAIService(client=openai_client)
            return

        if llm_setting.provider_type == "volcengine":
            ark_client = AsyncArk(
                api_key=llm_setting.api_key,
                base_url=llm_setting.base_url,
                timeout=llm_setting.timeout,
            )
            self.services[llm_setting.name] = VolcengineService(client=ark_client)
            return
        ...
        self.services[llm_setting.name] = GeminiService(client=gemini_client)
